...
MAX_TELEGRAM_MESSAGE_LENGTH = 4096  # лимит Telegram для обычных sendMessage

# Bot API 10.1 (июнь 2026): rich-сообщения (таблицы, LaTeX-формулы и т.п.)
# поддерживают значительно больший лимит — 32768 символов, с автоматической
# кнопкой «Показать ещё» после первых ~8000.
MAX_RICH_MESSAGE_LENGTH = 32768

# Включает отправку финального ответа как Rich Message (sendRichMessage),
# если в тексте обнаружены markdown-таблица или LaTeX-формула. При False —
# всегда используется старый HTML-рендер (markdown_to_telegram_html).
USE_RICH_MESSAGES = env.bool("USE_RICH_MESSAGES", default=True)
...
MAX_OUTPUT_TOKENS_TEXT = 1500
MAX_OUTPUT_TOKENS_CODE = 1000
...
Таблицы:
	•	Если данные удобно представить таблицей — используй обычный Markdown-синтаксис
	    таблиц (| колонка 1 | колонка 2 |, разделитель |---|---|). Telegram теперь
	    отображает их как настоящие таблицы.

Формулы:
	•	Формулы записывай в LaTeX: $...$ для формулы внутри строки текста
	    и $$...$$ для формулы отдельным блоком по центру. Telegram теперь
	    рендерит их нативно — не нужно заменять на Unicode-приближения.
	•	Если LaTeX неприменим (совсем простое выражение внутри обычного предложения),
	    можно использовать Unicode-символы (E = mc², a² + b² = c²).
